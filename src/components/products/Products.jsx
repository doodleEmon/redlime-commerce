...
export default function ProductList(props) {
  ...
  const checkScrollable = () => {
    if (scrollRef.current) {
      ...
      console.log("isScrollableNow:", isScrollableNow);
  ...
  useEffect(() => {
    ...
    // Check scrollable on mount and when uniqueCategories changes
    checkScrollable();

    // Re-check scrollable after a short delay (e.g., 500ms)
    const timeoutId = setTimeout(checkScrollable, 500);

    // Re-check scrollable on window resize
  ...
  }, [uniqueCategories]); // Re-run effect when uniqueCategories changes
  ...
  return (
    <div className="max-sm:px-5 sm:px-10 md:px-20 lg:px-[100px] xl:px-[130px] max-sm:mt-[50px] sm:mt-[80px] md:mt-[100px] lg:mt-[140px] xl:mt-[160px]">
      ...
      {/* Filter Section */}
      <div className="relative flex items-center justify-center mb-4 mt-11 max-sm:px-[35px] sm:px-[60px] md:px-[75px] lg:px-[85px] xl:px-[95px]">
        {isScrollable && scrollPosition > 0 && (
          <button
            onClick={() => handleScroll("left")}
            className={`absolute left-0 ${
              btnClicked ? "bg-[#E1C06E]" : "bg-blue-950 bg-opacity-60"
            } border max-sm:px-4 sm:px-5 md:px-6 lg:px-7 xl:px-8 max-sm:py-2 md:py-[18px] lg:py-[20px] xl:py-[22.5px] rounded-full z-10`}
          >
            <img
              src={btnClicked ? leftArrBlack : leftArrBtn}
              alt="Left arrow button"
              className="max-sm:w-4 sm:w-5 md:w-6"
            />
          </button>
        )}
        <div
        ...
        >
          {uniqueCategories.map((category, index) => (
            <button
              key={index}
              className={`max-sm:px-3 max-sm:py-1 sm:px-4 sm:py-2 md:px-5 md:py-3 lg:px-6 lg:py-4 rounded-full whitespace-nowrap ${
            ...
            >
              <p className="max-sm:text-[12px] sm:text-[14px] md:text-[16px] lg:text-[18px] capitalize">
                {category}
              </p>
            </button>
          ))}
